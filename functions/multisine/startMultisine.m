...
if ~isempty(multisine.session.Channels) &&  ~isempty(multisine.channelInfo.reference)
    ...
    for I = 1:K
        fprintf('____ FREQUENCY STEP %u/%u ____\n', I, K)
        %%% EXPERIMENTAL FEATURE
        coherenceValue = 0;
        minCoherenceValue = 0.95; % good value around 0.5 - 0.6
        stepLoadFactor = 1.5; % small increments take longer time (1.25 good)
        maxFactorIncrease = 2.3; % don't want to damadge the shaker (2 good)
        timesIncrease = log(maxFactorIncrease)/log(stepLoadFactor);
        firstCohRun = true;
        cohRun = 1;
        while any(find(coherenceValue < minCoherenceValue)) || firstCohRun == true
            fprintf('____ Coherence run %u ____\n', cohRun)
            
            coherenceValue = 0; % must be reset after every loop start
            ...
            while iret == -1
                ...
                if multisine.session.IsRunning == 0
                    ...
                    nidaqMultisinePutSineDataInline(multisine.session, [])
                    disp('Was 0 in stationary check, restarting')
                    startBackground(multisine.session);
                    pause(0.1);
                end
            ...
            for JJ=1:Nstat
                ...
                while iret == -1
                    ...
                    if multisine.session.IsRunning == 0
                        ...
                        multisine.session.stop();
                        nidaqMultisinePutSineDataInline(multisine.session, [])
                        disp('Was 0 in statistics, checking stationarity again')
                        startBackground(multisine.session);
                        pause(0.1);
                        Ctmean = Ct;
                        CtmeanChanged = true;
            ...
            for indii = 1:length(indf)
                ...
                if coherenceValue(:,indii) < minCoherenceValue
                    ...
                    if stepLoadFactor*loads(indf(indii)) < stepLoadFactor^timesIncrease*loadsDefault
                        ampChanged = 1;
                        loads(indf(indii)) = loads(indf(indii))*stepLoadFactor;
                    else
                        coherenceValue(:,indii) = minCoherenceValue;
                        
                        indfKeep(indiiC) = indii;
                        indiiC = indiiC + 1;
                    end
                else
                    indfKeep(indiiC) = indii;
                ...
                if ampChanged == 1
                    fprintf('Min coh. for freq %0.2f is %0.2f, and amplitude %0.4f (default %0.4f) amp INCREASED.\n',Freqs(indf(indii)),coherenceValue(indii),loads(indf(indii)),loadsDefault)
                end
                
                coherenceAmplitude(indf(indii)) = coherenceValue(indii);
            end
            
            indfiNew = 1:length(indf); indfiNew = setdiff(indfiNew,indfKeep);
            indfNew = setdiff(indf,indf(indfKeep));
            indf = indf(indfKeep); 
            tStart = Ts;
            blockSizeInPutData = blockSize;
            w = Freqs(indfNew);
        ...
        % Pass data
        PassDoubleThruFile(MMF{4},indfOrig,1); % pass indf data
        %PassDoubleThruFile(MMF{4},indf,1); % pass indf data
        PassDoubleThruFile(MMF{5},real(frdsys.ResponseData(:,:,indfOrig)),1); % pass H data
        PassDoubleThruFile(MMF{6},imag(frdsys.ResponseData(:,:,indfOrig)),1); % pass H data
        PassDoubleThruFile(MMF{7},C,1); % pass C data
    end
    ...
    multisine.Metadata.AmplitudeLevel = loads;
...
clear -global dataObject
    ...
    function nidaqMultisinePutSineDataInline(src, event)
        ...
        for II = 1:length(indf)
            u = u + loads(indf(II)).* sin(om(II) * tArg + fi(II));
        end
