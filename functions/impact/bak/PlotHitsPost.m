...
if plotch==refch
   plotch=setdiff(1:length(ChNames),refch);
   plotch=plotch(1);
end   
...
try
  for I=1:length(Y)
    t=Y{I}(end,:)-Y{I}(end,1);
    yp=Y{I}(plotch,:); yp=yp-median(yp); yp=yp/norm(yp,inf);
...
FRD.OutputName=Impact.Metadata.Sensor.Label(respch);
