function [uff,rpt]=abraDAQterm(TSsys,FRDsys)
[hf,hui]=terminalGUI(TSsys,FRDsys);
while 1
  if get(hui(31),'UserData'),break,end
  pause(.1)
end
...
if get(hui(13),'Value')
  ...
  try magphase([1 1;f0 f1],FRDsys); catch, warning('magphase.m not found in search path'),end
end  
...
if get(hui(22),'Value')
  ...
  uff=false;
%   [filen,pathen] = uiputfile('*.uff','Universal File Save');
%   if filen
%     filen=[pathen filen];
%     if get(hui(21),'Value')
%       frd2uff(filen,FRDmob,'replace');
%     else
%       frd2uff(filen,FRDsys,'replace');
%     end
%   end  
end
...
hui(31)=uicontrol('Parent',hp(3),'Style','Push','String','Done',...
  'Position',[8 15 50 20],'BackgroundColor',[.9 .9 .9], ...
  'CallBack','set(gco,''UserData'',true)');
