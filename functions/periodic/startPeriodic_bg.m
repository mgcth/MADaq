...
if ~isempty(periodic.session.Channels) && ~isempty(periodic.channelInfo.reference)
    ...
    if ~strcmpi(char(get(handles.fun3,'String')),'multisine')
        ...
        periodic.session.NotifyWhenDataAvailableExceeds=Ndata;
        periodic.session.NotifyWhenScansQueuedBelow=ceil(Ndata/2);

        
        %%                                           Find appropriate load level
        LowVolt=0.02;
        qd=LowVolt*Load(:)/norm(Load(:),'inf');
        periodic.session.queueOutputData(qd);
        startBackground(periodic.session);
        haveData=false;
        for I=1:2,% One extra for electronics to warm up
          while ~haveData, pause(0.05); end
          haveData=false;
        end
        daq.reset;
        u=y(:,Refch)*ucal;
        ...
        %%                                      Start pumping ot periodic signal
        qd=LoadFactor*Load(:);
        % Add listeners        
        periodic.session.queueOutputData(qd);
        startBackground(periodic.session);
...
%         %wait(periodic.session)
        ...
        frdsys.OutputName=periodic.Metadata.Sensor.Label(yind);
