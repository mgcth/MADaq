...
if nargin==0
    ...
    sssGUI(CH.active); %sssGUI(CH.active);
    ...
    if plotopt{1}; % FRF plot
        ...
        if ~isempty(abs(FRFplus(chplot,:))) || ~isempty(abs(FRFminus(chplot,:))) || ~isempty(abs(FRF(chplot,:)))
            ...
            title(([names(chplot) ' / ' names(find(CH.active==CH.reference))])); %title(cell2mat([cell2mat(names(chplot)) '/ ' names(find(CH.active==CH.reference))]))
    ...
    if get(HFRFGUI.RB(5),'Value')
        ...
        titletext=[names(chplot) ' @ ' num2str(freq(Ind)) 'Hz'];%titletext=[cell2mat(names(chplot)) ' @ ' num2str(freq(Ind)) 'Hz'];
