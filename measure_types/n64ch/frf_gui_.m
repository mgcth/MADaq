function frf_gui(t,y,Ind,freq,FRF,stdFRF,mlFRF,RN,RH,RS,PW,names)
%FRF_GUI
%Alt 1:  
...
%      semilogy(freq(1:Nf),abs(FRF(chplot,:)))
      loglog(freq(1:Nf),abs(FRFplus(chplot,:)),'color',grey)
      ax=axis;axis([freq(1) freq(end) ax(3)/5 ax(4)]);
      hold on
      loglog(freq(1:Nf),abs(FRFminus(chplot,:)),'color',grey)     
      loglog(freq(1:Nf),abs(FRF(chplot,:)))
  ...
  elseif plotopt{3}; % Nyquist plot
     ...
     Nf=size(FRF,2);
     figure(HFRFGUI.hFigfd);
     Chplot=chplot(1);
     loglog(freq(1:Nf),squeeze(abs(mlFRF(Chplot,:,:))))
     ax=axis;axis([freq(1) freq(end) ax(3)/5 ax(4)]);
     title(cell2mat([cell2mat(names(Chplot)) '/ ' names(find(CH.active==CH.refch))]))
  end
